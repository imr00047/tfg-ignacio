    ...
    std::pair<Spectrum, Mask> sample(const Scene *scene, Sampler *sampler,
        ...
        for (int depth = 1;; ++depth) {
            ...
            if (likely(any_or<true>(active_e))) {
                ...
                Float bsdf_pdf_reverse = Float(1.f) - bsdf_pdf;
                bsdf_pdf_reverse       = pow(bsdf_pdf_reverse, Float(10.0));
                bsdf_pdf               = Float(1.f) - bsdf_pdf_reverse;

                Float mis =
                    select(ds.delta, Float(1.f), mis_weight(ds.pdf, bsdf_pdf));

                result[active_e] += bsdf_pdf * emitter_val * mis * throughput;
            }

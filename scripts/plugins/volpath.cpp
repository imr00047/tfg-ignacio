        ...
        for (int bounce = 0;; ++bounce) {
            ...
            if (any_or<true>(active_surface)) {
                ...
                if (likely(any_or<true>(active_e))) {
                    ...
                    Vector3f wo = si.to_local(ds.d) ; // Vector3f(0.f, 0.f, 1.f);
                    ...
                    result[active_e] += throughput * mis_weight(ds.pdf, select(ds.delta, 0.f, bsdf_pdf)) * emitted;
                }
                ...
                result += select(add_emitter && neq(emitter_pdf, 0), mis_weight(bs.pdf, emitter_pdf) * throughput * emitted, 0.0f);
